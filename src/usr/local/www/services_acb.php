...
if ($_REQUEST['rmver'] != "") {
	...
	curl_setopt($curl_session, CURLOPT_URL, "https://konntrol.com.br/rmbkp");
	...
	if (curl_errno($curl_session)) {
		...
		fwrite($fd, "https://konntrol.com.br/rmbkp" . "" . "action=delete&hostname=" . urlencode($hostname) . "&revision=" . urlencode($_REQUEST['rmver']) . "\n\n");
		fwrite($fd, $data);
...
if ($_REQUEST['newver'] != "") {
	...
	if (curl_errno($curl_session)) {
		...
		fwrite($fd, "https://konntrol.com.br/rmbkp" . "" . "action=restore&hostname={$hostname}&revision=" . urlencode($_REQUEST['newver']) . "\n\n");
		fwrite($fd, $data);
		fwrite($fd, curl_error($curl_session));
		fclose($fd);
	} else {
	...
	if (!$input_errors && $data) {
		...
		if (config_restore("/tmp/config_restore.xml") == 0) {
			$savemsg = "Successfully reverted the KONTROL-UTM configuration to revision " . urldecode($_REQUEST['newver']) . ".";
			$savemsg .= <<<EOF
			<br />
...
if ($_REQUEST['download']) {
	...
	if (!tagfile_deformat($data, $data1, "config.xml")) {
		$input_errors[] = "The downloaded file does not appear to contain an encrypted Konntrol configuration.";
	} else {
		$ds = explode('++++', $data);
		...
		if (!tagfile_deformat($data, $data, "config.xml")) {
			$input_errors[] = "The downloaded file does not appear to contain an encrypted KONTROL-UTM configuration.";
		}
...
if ( !($_REQUEST['download']) || $input_errors) {
	...
	curl_setopt($curl_session, CURLOPT_URL, "https://konntrol.com.br/list");
	curl_setopt($curl_session, CURLOPT_POSTFIELDS, "userkey=" . $userkey .
	...
	if (curl_errno($curl_session)) {
		...
		fwrite($fd, "https://konntrol.com.br/list" . "" . "action=showbackups" . "\n\n");
		fwrite($fd, $data);
		fwrite($fd, curl_error($curl_session));
		fclose($fd);
	} else {
